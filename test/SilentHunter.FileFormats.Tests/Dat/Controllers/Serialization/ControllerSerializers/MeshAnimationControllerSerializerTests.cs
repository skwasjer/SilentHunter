...
public class MeshAnimationControllerSerializerTests
{
    ...
    {
        byte[] expectedSerializedData = [0x02, 0x00, 0x48, 0xe1, 0x7a, 0x3f, 0x00, 0x00, 0x00, 0x00, 0xc0, 0x3f, 0x01, 0x00, 0x02, 0x00, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0xc0, 0x00, 0xc0, 0x38, 0xc0, 0x00, 0x40, 0x40, 0xab, 0xaa, 0x55, 0xd5, 0x00, 0x00, 0xaa, 0x2a, 0x55, 0x55, 0xff, 0x7f, 0xc0, 0x00, 0xc0, 0x38, 0x40, 0xff, 0x3f, 0xc0, 0x55, 0xd5, 0xab, 0xaa, 0x00, 0x80, 0x55, 0x55, 0xaa, 0x2a, 0x00, 0x00, 0x10, 0xff, 0xc0];
        _controller.Frames =
        [
            new AnimationKeyFrame { FrameNumber = 0, Time = 0.98f }, new AnimationKeyFrame { FrameNumber = 1, Time = 1.5f }
        ];
        _controller.CompressedFrames =
        [
            VectorCompression.Compress(
                new List<Vector3> { new(1, 2, 3), new(4, 5, 6) }
            ),

            VectorCompression.Compress(
                new List<Vector3> { new(-4, -5, -6), new(-1, -2, -3) }
            )
        ];
        _controller.Unknown0 = [0x10, 0xFF, 0xC0];
